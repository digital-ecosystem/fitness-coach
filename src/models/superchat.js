...
export async function sendMessage(templateId, video_id, contact_id) {
  const options = {
    ...
    body: JSON.stringify({
      ...
      content: {type: 'whats_app_template', template_id: templateId, file: {id: video_id}}
    })
  };
  
  return fetch('https://api.superchat.com/v1.0/messages', options)
  .then(response => response.json())
  .catch(err => console.error("something went wrong wiht sending the message through Superchat", err));
}
